...
fn spice_all_queries() -> Result<()> {
    ...
    tokio::runtime::Builder::new_multi_thread()
        ...
        .enable_all()
        .build()?
        .block_on(async { run_spice_suite(SPICE_SOURCE_CK, TEST_ITERATIONS, TEST_WORKER_THREADS).await })
}
...
fn native_all_queries() -> Result<()> {
    ...
    tokio::runtime::Builder::new_multi_thread()
        ...
        .enable_all()
        .build()?
        .block_on(async { run_native_suite_clickhouse(TEST_ITERATIONS, TEST_WORKER_THREADS).await })
}
